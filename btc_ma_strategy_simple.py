"""
BTC期货交易策略 - 基于MA120和MA360均线系统 (简化版)
"""
...
def calculate_ma_signals(data):
    ...
    df['MA360_prev'] = df['MA360'].shift(1)
    
    # 金叉: MA120从下方突破MA360
    golden_cross = (df['MA120'] > df['MA360']) & (df['MA120_prev'] <= df['MA360_prev'])
    df.loc[golden_cross, 'MA_Signal'] = 1
    
    # 死叉: MA120从上方跌破MA360
    death_cross = (df['MA120'] < df['MA360']) & (df['MA120_prev'] >= df['MA360_prev'])
...
def analyze_results(results, initial_capital):
    ...
    if len(equity_curve) > 1:
        ...
        if len(returns) > 0:
            ...
            if volatility > 0:
                sharpe_ratio = annualized_return / volatility if 'annualized_return' in locals() else 0
...
def plot_results(results, save_path='btc_ma_strategy_results.png'):
    """绘制结果图表"""
    data = results['data']
    equity_curve = results['equity_curve']
    trades = results['trades']
    ...
    # 2. 权益曲线
    axes[1].plot(data.index, equity_curve, label='策略权益', color='blue')
    axes[1].axhline(y=results['final_capital'] - sum([t.get('pnl', 0) for t in trades]), 
                    color='gray', linestyle='--', alpha=0.5, label='初始资金')
    
    # BTC买入持有基准
    initial_capital = results['final_capital'] - sum([t.get('pnl', 0) for t in trades])
    btc_value = initial_capital * (data['Close'] / data['Close'].iloc[0])
    ...
    # 3. RSI指标
    axes[2].plot(data.index, data['RSI'], color='purple', alpha=0.7)
...
def main():
    ...
    try:
        ...
        print("计算技术指标...")
        data_with_signals = calculate_ma_signals(btc_data)
        
        print("开始回测...")
        results = backtest_strategy(data_with_signals, initial_capital, stop_loss, take_profit)
        ...
        if results['trades']:
            ...
            trades_df.to_csv('btc_ma_trades_simple.csv', index=False)
            print("交易记录已保存至: btc_ma_trades_simple.csv")
        
        # 保存详细数据
        data_with_signals['Equity'] = results['equity_curve']
        data_with_signals.to_csv('btc_ma_strategy_data_simple.csv')
        print("详细数据已保存至: btc_ma_strategy_data_simple.csv")
